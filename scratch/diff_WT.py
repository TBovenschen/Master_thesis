...
output_file = pset.ParticleFile(name="/scratch/tycho/diff_model_WT.nc", outputdt=3600*24) # the file name and the time step of the outputs
...
#Execute  the kernel and write to file
output_file = pset.ParticleFile(name="/scratch/tycho/diff_model_WT_mean.nc", outputdt=3600*24) # the file name and the time step of the outputs
pset.execute(AdvectionRK4+age_kernel+sample_kernel,                 # the kernel (which defines how particles move)
             runtime=3600*24*16,    # the total length of the run
             dt=600,      # the timestep of the kernel
             recovery= {ErrorCode.ErrorOutOfBounds:deleteparticle}, #delete particle when it gets out of the domain
...
output = xr.open_dataset('/scratch/tycho/diff_model_WT.nc') #the hourly output data
mean_ds = xr.open_dataset('/scratch/tycho/diff_model_WT_mean.nc') #the data from the mean flow run



#load the mean flow field:
meanfield = xr.open_dataset('/scratch/tycho/SMOC_monthly_mean_2018.nc')
meanfield['v'] = meanfield.vo + meanfield.vtide #Mean flow of navier-stokes + tides
